...
//=============================================================================
	...
	// Mod By Syc;
	ON_NOTIFY_REFLECT(NM_RCLICK, &CXHtmlTree::OnNMRclick)
	ON_COMMAND(ID_CM_ADD_FOLDER, &CXHtmlTree::OnCmAddFolder)
	ON_COMMAND(ID_CM_ADD_EXT, &CXHtmlTree::OnCmAddExt)
	ON_COMMAND(ID_CM_DEL_ITEM, &CXHtmlTree::OnCmDelItem)
	ON_COMMAND(ID_CM_DEL_ITEMS, &CXHtmlTree::OnCmDelItems)
	ON_COMMAND(ID_CM_RENAME, &CXHtmlTree::OnCmRename)
	ON_WM_DROPFILES()
END_MESSAGE_MAP()
...
//=============================================================================
{
	...
	if( pTVDispInfo->item.pszText != NULL ) {
		...
		if (str[0] != '*' || str[1] != '.'){
			MessageBox(_T("文件类型必须以 *. 开头，谢谢合作！"), _T("提示"), MB_OK);
		...
		if ( iLen > str.GetLength() ) {
			MessageBox(_T("文件或文件夹名字中不能含有以下字符：/ \\ : * ? \" < > | \r\n文件类型必须以 *. 开头，谢谢合作！"), _T("提示"), MB_OK);
			DeleteItem(pTVDispInfo->item.hItem);
			*pResult = 1;
			return FALSE;
		}
...
void CXHtmlTree::OnNMRclick(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	SetForegroundWindow();
	if ( !hItem ) {
		pMenu->EnableMenuItem(ID_CM_ADD_FOLDER, MF_GRAYED);
		pMenu->EnableMenuItem(ID_CM_ADD_EXT, MF_GRAYED);
		pMenu->EnableMenuItem(ID_CM_DEL_ITEM, MF_GRAYED);
		pMenu->EnableMenuItem(ID_CM_RENAME, MF_GRAYED);
	}
...
/*** Mod by Syc start ***/
void CXHtmlTree::OnCmAddFolder()
{
	int nImage = -1, nSelectedImage = -1;
	HTREEITEM hSelect = GetSelectedItem();
	if ( hSelect ) {
		GetItemImage(hSelect, nImage, nSelectedImage);
		if ( nImage == FILE_IMAGE ) {
			MessageBox(_T("文件类型下面不允许添加子结点！"), _T("提示"), MB_OK);
			return;
		}
	}
...
void CXHtmlTree::OnCmDelItem()
{
	HTREEITEM hSelect = GetSelectedItem();
	if ( !hSelect ) {
		return;
	}
	int iImg = -1, iSelectImg = -1, iRet = -1;
	GetItemImage(hSelect, iImg, iSelectImg);
	if ( iImg == FOLDER_CLOSE_IMAGE ) {
		iRet = MessageBox(_T("此结点为文件夹类型，若删除此结点将删除其下所有子结点！\r\n确认删除此结点吗？"), _T("提示"), MB_OKCANCEL | MB_ICONQUESTION);
	}else{
		iRet = MessageBox(_T("确认删除此结点吗？"), _T("提示"), MB_OK | MB_ICONQUESTION);
	}
	if ( hSelect && iRet == IDOK) {
		DeleteItem(hSelect);
	}
}

void CXHtmlTree::OnCmDelItems()
{
	int iRet = MessageBox(_T("确认删除所有结点吗？"), _T("提示"), MB_OKCANCEL | MB_ICONQUESTION);
	if ( iRet == IDOK ) {
		DeleteAllItems();
	}
}

void CXHtmlTree::OnCmRename()
{
	HTREEITEM hSelect = GetSelectedItem();
	if ( hSelect ) {
		EditLabel(hSelect);
...
/******* Mod end ********/
void CXHtmlTree::OnDropFiles(HDROP hDropInfo)
{
	TCHAR szFileName[MAX_PATH] = {0};
	int iFileNumber = 0;
	CXTipComboBox *pComBox = &(((CConfigDlg *)m_pParentWnd)->m_TreeSelect);
	...
	for (int i = 0; i < iFileNumber; i++) {
		...
		if ( dwAttr != INVALID_FILE_ATTRIBUTES && !(dwAttr & FILE_ATTRIBUTE_DIRECTORY) ) {
			DeleteAllItems();
			BOOL bRet = LoadXmlFromFile(szFileName, CXHtmlTree::NoConvertAction);
			Expand(GetRootItem(), TVE_EXPAND);
			pComBox->SetCurSel(pComBox->InsertString(pComBox->GetCount()-1, szFileName));
		}
	}
...
void CXHtmlTree::SetParentWnd(CWnd *pWnd)
{
	m_pParentWnd = pWnd;
}