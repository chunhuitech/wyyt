...
BEGIN_MESSAGE_MAP(CXTipComboBox, CComboBox)
	...
	ON_WM_TIMER()
	//}}AFX_MSG_MAP
	ON_CONTROL_REFLECT(CBN_SELCHANGE, &CXTipComboBox::OnCbnSelchange)
	ON_CONTROL_REFLECT(CBN_DROPDOWN, &CXTipComboBox::OnCbnDropdown)
END_MESSAGE_MAP()
...
void CXTipComboBox::OnCbnSelchange()
{
	CString strText, strPath;
	GetWindowText(strText);
	if ( strText == _T("加载自定义列表 ...") ) {
		CXHtmlTree *pTree = &(((CConfigDlg *)m_pParentWnd)->m_PathTree);
		CFileDialog m_fileDlg(TRUE, _T("xml"), NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("Xml_Files (*.xml)|*.xml|All_Files (*.*)|*.*||"));
		if ( m_fileDlg.DoModal() == IDOK && pTree ) {
			strPath = m_fileDlg.GetPathName();
			int idx = FindString(-1, strPath);
			if ( idx != -1 ) {
				SetCurSel(idx);
			}else{
				SetCurSel(InsertString(GetCount()-1, strPath));
			}
		}else{
			SetCurSel(m_CurSel);
			return;
	...
	::EnableWindow(::GetDlgItem(m_pParentWnd->GetSafeHwnd(), IDC_APPLY_BTN), TRUE);
}

void CXTipComboBox::OnCbnDropdown()
{
	m_CurSel = GetCurSel();
}

void CXTipComboBox::SetParentWnd(CWnd *pWnd)
{
	m_pParentWnd = pWnd;
}
