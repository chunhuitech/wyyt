							...
							CResizablePage *pArchiveInfo, CResizablePage *pAttachInfo)
{
	if (pSheet->GetItems().GetSize() == 1)
	{
		...
		bool bNeedArchiveInfoPage = true;// = NeedArchiveInfoPage(&pSheet->GetItems());
		if (bNeedArchiveInfoPage)
		{
			...
			int iMediaInfoPage = pSheet->GetPageIndex(pAttachInfo);
			bool bMediaInfoPageWasActive = false;
			if (iMediaInfoPage >= 0) 
			{
				if (pActivePage == pAttachInfo)
					bMediaInfoPageWasActive = true;
				if (!bUpdateWindow) {
					pSheet->SetRedraw(FALSE);
					bUpdateWindow = true;
				}
				pSheet->RemovePage(pAttachInfo);
			}
		...
		else
		{
			bool bFound = false;
			for (INT_PTR i = 0; i < pSheet->GetPages().GetSize(); i++) {
				if (pSheet->GetPages()[i] == pAttachInfo){
					bFound = true;
					break;
				}
			}
			...
			if (!bFound) {
				...
				pSheet->InsertPage(iArchiveInfoPage, pAttachInfo);
				if (bArchiveInfoPageWasActive)
					pSheet->SetActivePage(pAttachInfo);
			}
		}
		if (bUpdateWindow) 
		{
