...
*/
/////////////////////////////////////////////////////////////////////////////
// CWyytSystrayDlg dialog

CWyytSystrayDlg::CWyytSystrayDlg(CWnd* pParent, CPoint pt, int iMaxUp, int iMaxDown, int iCurUp, int iCurDown)
	: CDialog(CWyytSystrayDlg::IDD, pParent)
{
...
CWyytSystrayDlg::~CWyytSystrayDlg()
{
	//if(m_hUpArrow)
...
void CWyytSystrayDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CWyytSystrayDlg)
...
BEGIN_MESSAGE_MAP(CWyytSystrayDlg, CDialog)
	//{{AFX_MSG_MAP(CWyytSystrayDlg)
	ON_WM_MOUSEMOVE()
	/* Xman
...
/////////////////////////////////////////////////////////////////////////////
// CWyytSystrayDlg message handlers

void CWyytSystrayDlg::OnMouseMove(UINT nFlags, CPoint point)
{	
...
BOOL CWyytSystrayDlg::OnInitDialog() 
{
...
*/
	...
	if(p)
	{
		p->GetFont()->GetLogFont(&lfStaticFont);//第一个控件获取下相关字体
		bValidFont = true;

		p->GetWindowRect(r);
		ScreenToClient(r);
		m_ctrlAbout.Create(NULL, NULL, WS_CHILD|WS_VISIBLE, r, this, IDC_ABOUT);
		m_ctrlAbout.m_nBtnID = IDC_ABOUT;
		//p->GetWindowText(m_ctrlRestore.m_strText);
		m_ctrlAbout.m_strText = GetResString(IDS_ABORT);
		m_ctrlAbout.m_strText.Remove(_T('&'));

		m_ctrlAbout.m_bUseIcon = true;
		m_ctrlAbout.m_sIcon.cx = 16;
		m_ctrlAbout.m_sIcon.cy = 16;
		m_ctrlAbout.m_hIcon = theApp.LoadIcon(_T("ABOUT"), m_ctrlAbout.m_sIcon.cx, m_ctrlAbout.m_sIcon.cy);
		m_ctrlAbout.m_bParentCapture = true;
		if(bValidFont)
		{	
			LOGFONT lfFont = lfStaticFont;
			lfFont.lfWeight += 200;			// make it bold
			m_ctrlAbout.m_cfFont.CreateFontIndirect(&lfFont);
		}	
	}
...
//	m_ctrlDownSpeedSld.SetPos(m_nDownSpeedTxt);
	...
	*/
	...
	m_ctrlSidebar.SetWindowText(SZ_FULL_VERSION);
...
/* Xman
void CWyytSystrayDlg::OnChangeDowntxt() 
{
	UpdateData();
...
void CWyytSystrayDlg::OnChangeUptxt() 
{
	UpdateData();
...
*/
void CWyytSystrayDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
...
void CWyytSystrayDlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
	ReleaseCapture();
...
void CWyytSystrayDlg::OnRButtonDown(UINT nFlags, CPoint point)
{
	CRect systrayRect;
...
void CWyytSystrayDlg::OnKillFocus(CWnd* pNewWnd) 
{
...
void CWyytSystrayDlg::OnShowWindow(BOOL bShow, UINT nStatus) 
{
...
void CWyytSystrayDlg::OnCaptureChanged(CWnd *pWnd) 
{
...
BOOL CWyytSystrayDlg::OnCommand(WPARAM wParam, LPARAM lParam) 
{
