...
wyytExtendDataManagePri::wyytExtendDataManagePri(void)
{
}
...
wyytExtendDataManagePri& wyytExtendDataManagePri::Instance()
{
	static wyytExtendDataManagePri s_WyytExtendDataManage;
	return s_WyytExtendDataManage;
}
int64 wyytExtendDataManagePri::GetSysClassMaxID()
{
	int64 nMaxID = 0;
...
void wyytExtendDataManagePri::InsertNewClassInfo( stSysClassInfoItem stNewCII )
{
	m_vecClassInfos.push_back(stNewCII);
	DataAccess::Instance().ISysClassOper->InsertClass(stNewCII, EMT_PRI);
}
void wyytExtendDataManagePri::ModClassInfo(int64 nID, wstring wstrNewClassName)
{
	for ( vector<stSysClassInfoItem>::iterator it = m_vecClassInfos.begin(); it != m_vecClassInfos.end(); it++ )
	{
		if ( it->ID == nID )
	...
	DataAccess::Instance().ISysClassOper->ModClass(nID, wstrNewClassName, EMT_PRI);
}

void wyytExtendDataManagePri::DelClassInfo(int64 nID)
{
	...
	//	}
	//};
	DataAccess::Instance().ISysClassOper->DelClass(nID, EMT_PRI);
}
void wyytExtendDataManagePri::SetClassInfoRecomm(int64 nID)
{
	vector<stSysClassInfoItem>::iterator it = find_if( m_vecClassInfos.begin(), m_vecClassInfos.end(), IsDelClass(nID) );
	if (it != m_vecClassInfos.end())
	{
	...
	DataAccess::Instance().ISysClassOper->SetClassRecomm(nID, 1, EMT_PRI);
}
bool wyytExtendDataManagePri::IsSysClassID(int64 nID)
{
...
bool wyytExtendDataManagePri::IfHaseSubClass(int64 nParentID)
{
	for(vector<stSysClassInfoItem>::iterator it = m_vecClassInfos.begin(); it != m_vecClassInfos.end(); it++ )
	{
...
wstring wyytExtendDataManagePri::GetClassFullName(int64 nCID)
{
	wstring strFullName;
...
bool wyytExtendDataManagePri::GetClassAllName(int64 nCID, vector<wstring>& vecAllNames)
{
...
void wyytExtendDataManagePri::CancleClassInfoType(int64 nID)
{
	vector<stSysClassInfoItem>::iterator it = find_if( m_vecClassInfos.begin(), m_vecClassInfos.end(), IsDelClass(nID) );
	if (it != m_vecClassInfos.end())
	{
	...
	DataAccess::Instance().ISysClassOper->SetClassRecomm(nID, 0, EMT_PRI);
}

void wyytExtendDataManagePri::InitSysClassIDs()
{
	for (int i = 0; i < 1/*ROOT_CLASS_COUNTS*/; i++ )
	{
...
//void wyytExtendDataManagePri::Init()
//{
//	DataAccess::Instance().InitDb();
//}
//void wyytExtendDataManagePri::Unit()
//{
//	DataAccess::Instance().UnitDb();
//}
//void wyytExtendDataManagePri::Reload()
//{
//	ClearClassInfoDatas();
//	InitClassInfoDatas();
//
...
void wyytExtendDataManagePri::ClearClassInfoDatas()
{
	m_vecClassInfos.clear();
}

void wyytExtendDataManagePri::InitClassInfoDatas()
{
	 //ExportClassInfos( m_vecClassInfos); //统一由工厂方法访问数据库接口
	//I_T_SysClassInfos_Oper* pSysClssInfoOper = DataAccess::CreateSysClassOper();
	DataAccess::Instance().ISysClassOper->GetAllClass(m_vecClassInfos, EMT_PRI);
...
void wyytExtendDataManagePri::ClearSysRecordInfoDatas()
{
	m_vecRecordItems.clear();
}

void wyytExtendDataManagePri::InitSysRecordInfoDatas()
{
	//vector<CRecordItem>  vecRecordItems;
	DataAccess::Instance().ISysClassOper->GetSysRecordInfos(m_vecRecordItems, EMT_PRI);
	//for ( vector<CRecordItem>::iterator it = m_vecRecordItems.begin(); it != m_vecRecordItems.end(); it++ )
	//{
	//	it->nAllClassID.reserve(10);
	//}
	DataAccess::Instance().ISysClassOper->GetSysRecToClassInfos_DB(m_vecRecordItems, EMT_PRI);
...
void wyytExtendDataManagePri::ModSysRecordInfo( CRecordItem* record )
{
	DataAccess::Instance().ISysClassOper->DelSysRecToClass_DB(record->ID, EMT_PRI);
	for (vector<int64>::iterator it = record->nAllClassID.begin(); it != record->nAllClassID.end(); it++ )
	{
		DataAccess::Instance().ISysClassOper->InsertSysRecToClass_DB(record->ID,(*it), EMT_PRI);
	}
	DataAccess::Instance().ISysClassOper->ModSysRecordInfo_DB(record, EMT_PRI);
}

void wyytExtendDataManagePri::DelSysRecordAttachInfo(int64 nRID, bool bDelAttachFile)
{
//	DelSysRecordAttachFile();
	DataAccess::Instance().ISysClassOper->DelSysRecordAttachInfo_DB(nRID, EMT_PRI);
...
bool wyytExtendDataManagePri::DelSysRecordAttachFile(const WCHAR * srcPathFile)
{
...
bool wyytExtendDataManagePri::AddSysRecordAttachFile(const WCHAR * srcPath, const WCHAR * destPath)
{
	return ::CopyFile(srcPath,destPath,FALSE);
}
bool wyytExtendDataManagePri::MoveSysRecordAttachFile(const WCHAR * srcPath, const WCHAR * destPath)
{
	return ::MoveFile(srcPath,destPath);
}
void wyytExtendDataManagePri::DelSysRecordInfo(int64 nRID)
{
	//删除记录对应的分类
	DataAccess::Instance().ISysClassOper->DelSysRecToClass_DB(nRID, EMT_PRI);
	//删除记录对应的附件
	...
	DataAccess::Instance().ISysClassOper->DelSysRecordInfo_DB(nRID, EMT_PRI);
}
void wyytExtendDataManagePri::InsertSysRecordInfo( CRecordItem record )
{
	//DataAccess::Instance().ISysClassOper->GetSysRecordMaxID(record.ID); //单独调用以支持新加的数据记录获取ID以生成SID
	DataAccess::Instance().ISysClassOper->InsertSysRecordInfo_DB(record, EMT_PRI);
	DataAccess::Instance().ISysClassOper->DelSysRecToClass_DB(record.ID, EMT_PRI);
	for (vector<int64>::iterator it = record.nAllClassID.begin(); it != record.nAllClassID.end(); it++ )
	{
		DataAccess::Instance().ISysClassOper->InsertSysRecToClass_DB(record.ID,(*it), EMT_PRI);
...
void wyytExtendDataManagePri::GetSysRecordMaxID(int64& nID)
{
	DataAccess::Instance().ISysClassOper->GetSysRecordMaxID(nID, EMT_PRI);
}


void wyytExtendDataManagePri::ClearSysRecAttachsDatas()
{
	m_mapRec2Attachs.clear();
}

void wyytExtendDataManagePri::InitSysRecAttachsDatas()
{
	vector<CRecordAttachs> vecAllAttachs;
	DataAccess::Instance().ISysClassOper->GetSysRecAttachsInfos_DB(vecAllAttachs, EMT_PRI);
...
bool wyytExtendDataManagePri::InsertSysRecordAttachInfo( CRecordAttachs AttInfo )
{
	...
	//{
		//添加到数据库
		DataAccess::Instance().ISysClassOper->InsertSysRecordAttachInfo_DB( AttInfo, EMT_PRI );
