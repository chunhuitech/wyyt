...
BOOL CListViewWalkerPropertySheet::OnInitDialog()
{
	...
	if (m_pListCtrl != NULL)
	{
		...
		static const int _aiPropSheetButtonsStr[] = { IDS_TREEOPTIONS_OK, IDS_CANCEL, IDS_PW_APPLY, IDS_EM_HELP };// X: [AL] - [Additional Localize]
		...
		for (size_t i = 0; i < ARRSIZE(aCtrls); i++)
		{
			...
			//隐藏 上下条按钮
			aCtrls[i].pCtlBtn->ShowWindow(SW_HIDE);
		}
	}
