...
IMPLEMENT_DYNAMIC(CPrivateSharedRecordsCtrl, CMuleListCtrl)

BEGIN_MESSAGE_MAP(CPrivateSharedRecordsCtrl, CMuleListCtrl)
...
CPrivateSharedRecordsCtrl::CPrivateSharedRecordsCtrl()
...
void CPrivateSharedRecordsCtrl::Init()
{
	//设置配置文件preferences.ini中关于列表控件相关属性(排序、列宽等)的前缀名称
	SetPrefsKey(_T("PriRecordCtrl"));
...
void CPrivateSharedRecordsCtrl::OnSysColorChange()
{
...
void CPrivateSharedRecordsCtrl::SetAllIcons()
{
...
void CPrivateSharedRecordsCtrl::Localize()
{
...
void CPrivateSharedRecordsCtrl::AddRecord(const CRecordItem* record)
{
...
void CPrivateSharedRecordsCtrl::AddFile(const CShareableFile* file)
{
	if (!CwyytDlg::IsRunning())
		return;
...
void CPrivateSharedRecordsCtrl::RemoveFile(const CShareableFile* file, bool bDeletedFromDisk)
{
...
void CPrivateSharedRecordsCtrl::UpdateFile(const CShareableFile* file, bool bUpdateFileSummary/*, bool force*/) //Xman advanced upload-priority
{
...
void CPrivateSharedRecordsCtrl::UpdateRecord(const CRecordItem* record, bool bUpdateFileSummary/*, bool force=false*/)
{
	if (!record || !CwyytDlg::IsRunning())
		return;
}
int CPrivateSharedRecordsCtrl::FindRecord(const CRecordItem* pRecord)
{
	LVFINDINFO find;
...
int CPrivateSharedRecordsCtrl::FindFile(const CShareableFile* pFile)
{
	LVFINDINFO find;
	find.flags = LVFI_PARAM;
...
void CPrivateSharedRecordsCtrl::ReloadRecordList()
{
	DeleteAllItems();
	AddRecordDatasByClassIDs();
	//AddRecordDatas();
}
void CPrivateSharedRecordsCtrl::ReloadRecordList(vector<int64> nVecSelClassIDs)
{
	DeleteAllItems();
	AddRecordDatas_PreDeal(nVecSelClassIDs);
}
//void CPrivateSharedRecordsCtrl::ReloadFileList()
//{
//	DeleteAllItems();
//}

void CPrivateSharedRecordsCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
void CPrivateSharedRecordsCtrl::GetItemDisplayText(const CRecordItem* record, int iSubItem, LPTSTR pszText, int cchTextMax) const
{
...
void CPrivateSharedRecordsCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
...
BOOL CPrivateSharedRecordsCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
...
void CPrivateSharedRecordsCtrl::OnLvnColumnClick(NMHDR *pNMHDR, LRESULT *pResult)
{
...
int CPrivateSharedRecordsCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
...
void CPrivateSharedRecordsCtrl::OpenFile(const CShareableFile* file)
{
...
void CPrivateSharedRecordsCtrl::OnNmDblClk(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
...
void CPrivateSharedRecordsCtrl::CreateMenues()
{
	//if (m_PrioMenu) VERIFY( m_PrioMenu.DestroyMenu() );
	//if (m_CollectionsMenu) VERIFY( m_CollectionsMenu.DestroyMenu() );
	//if (m_SharedFilesMenu) VERIFY( m_SharedFilesMenu.DestroyMenu() );
...
void CPrivateSharedRecordsCtrl::ShowSelectedFileComments()
{
	POINT point;
	::GetCursorPos(&point);
	CPoint p = point; 
...
void CPrivateSharedRecordsCtrl::OnLvnGetDispInfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	if (CwyytDlg::IsRunning()) {
...
void CPrivateSharedRecordsCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void CPrivateSharedRecordsCtrl::ShowFileDialog(CAtlList<CRecordItem*>& aFiles, UINT uPshInvokePage)
{
	if (aFiles.GetCount() > 0)
	{
		CSharedFileDetailsSheet dialog(aFiles, uPshInvokePage, this,EMT_PRI);
		dialog.DoModal();
	}
}


void CPrivateSharedRecordsCtrl::SetClassFilter(CClassInfoItem* pNewFilter, bool bRefresh)
{
...
void CPrivateSharedRecordsCtrl::SetClassFilter(vector<int64> nVecSelClassIDs, bool bRefresh)
{
	
	ReloadRecordList(nVecSelClassIDs);
	
}
//void CPrivateSharedRecordsCtrl::SetDirectoryFilter(CDirectoryItem* pNewFilter, bool bRefresh){
...
void CPrivateSharedRecordsCtrl::OnLvnGetInfoTip(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CPrivateSharedRecordsCtrl::SetToolTipsDelay(DWORD dwDelay)
{
	CToolTipCtrl* tooltip = GetToolTips();
	if (tooltip)
...
bool CPrivateSharedRecordsCtrl::IsSharedInKad(const CKnownFile *file) const
{
	return false;
...
void CPrivateSharedRecordsCtrl::AddSelClassIDbyCalc(int64 nID)
{
...
void CPrivateSharedRecordsCtrl::GetPCID2AllSubClassIDs(int64 nParentCID)
{
...
void CPrivateSharedRecordsCtrl::AddRecordDatas_PreDeal(vector<int64> nVecSelClassIDs)
{
...
bool CPrivateSharedRecordsCtrl::HasSelectedOrSubClass(int64 nCID)
{
...
void CPrivateSharedRecordsCtrl::AddRecordDatasByClassIDs()
{
	...
	SetRedraw(FALSE);
	for (vector<CRecordItem>::iterator it = wyytExtendDataManagePri::Instance().m_vecRecordItems.begin(); it !=  wyytExtendDataManagePri::Instance().m_vecRecordItems.end(); it++ )
	{
		bool bAdd = false;
...
void CPrivateSharedRecordsCtrl::AddRecordDatas()
{
	//先清除原来的所有记录
...
void CPrivateSharedRecordsCtrl::AddShareableFiles(CString strFromDir)
{
	////while (!liTempShareableFilesInDir.IsEmpty())	// cleanup old filelist
	////	delete liTempShareableFilesInDir.RemoveHead();
...
BOOL CPrivateSharedRecordsCtrl::OnNMClick(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CPrivateSharedRecordsCtrl::CheckBoxClicked(int iItem)
{
	if (iItem == (-1))
	{
		ASSERT( false );
...
bool CPrivateSharedRecordsCtrl::CheckBoxesEnabled() const
{
	return false;
...
void CPrivateSharedRecordsCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
...
CPrivateSharedRecordsCtrl::CShareDropTarget::CShareDropTarget()
{
	m_piDropHelper = NULL;
	m_pParent = NULL;
...
DROPEFFECT CPrivateSharedRecordsCtrl::CShareDropTarget::OnDragEnter(CWnd* pWnd, COleDataObject* pDataObject, DWORD /*dwKeyState*/, CPoint point)
{
...
DROPEFFECT CPrivateSharedRecordsCtrl::CShareDropTarget::OnDragOver(CWnd* /*pWnd*/, COleDataObject* pDataObject, DWORD /*dwKeyState*/, CPoint point)
{
	DROPEFFECT dwEffect = DROPEFFECT_NONE;
...
BOOL CPrivateSharedRecordsCtrl::CShareDropTarget::OnDrop(CWnd* /*pWnd*/, COleDataObject* pDataObject, DROPEFFECT dropEffect, CPoint point)
{
...
void CPrivateSharedRecordsCtrl::CShareDropTarget::OnDragLeave(CWnd* /*pWnd*/)
{
    if (m_bUseDnDHelper)
        m_piDropHelper->DragLeave();
}
void CPrivateSharedRecordsCtrl::RecordDelete( int64 ID )
{
	//先删除附件信息
	map<int64, vector<CRecordAttachs>>::iterator Iter = wyytExtendDataManagePri::Instance().m_mapRec2Attachs.find(ID);
	if (Iter != wyytExtendDataManagePri::Instance().m_mapRec2Attachs.end() )
	{
		for (vector<CRecordAttachs>::iterator it = Iter->second.begin(); it != Iter->second.end(); it++ )
		{
			...
			csFileFullName.Format(L"%s\\%s%s",/*thePrefs.GetMuleDirectory(WYYT_ATTACHMENTS_PRIVATE_DIR)*/thePrefs.m_strPriAttachPath,it->CodeID.c_str(),it->FileSuffix.c_str());
			wyytExtendDataManagePri::Instance().DelSysRecordAttachFile(csFileFullName);
		}

	}
	//再删除数据库及内存信息
	wyytExtendDataManagePri::Instance().DelSysRecordInfo(ID);
}
void CPrivateSharedRecordsCtrl::RecordDelete( vector<int64> vecSelected )
{
...
void CPrivateSharedRecordsCtrl::RecordDelete( const CRecordItem* pRecord )
{
...
int CPrivateSharedRecordsCtrl::ShowRecDetailsDlg_MOD(CRecordItem* pRecord)
{
	CRecordDetails dlgDetails(NULL,EMT_PRI);	
	...
	if(dlgDetails.DoModal()  == IDOK)
	{
		...
		wyytExtendDataManagePri::Instance().ModSysRecordInfo(pRecord);
...
int CPrivateSharedRecordsCtrl::ShowRecDetailsDlg_ADD()
{
	CRecordItem stRI;
	CRecordDetails dlgDetails(NULL,EMT_PRI);;	
	dlgDetails.SetCurOperOption(ECOO_ADD);
	wyytExtendDataManagePri::Instance().GetSysRecordMaxID(stRI.ID);
	dlgDetails.InitCurRecordInfo(&stRI);
	if(dlgDetails.DoModal()  == IDOK)
	{
		CSID CodeID(E_SMT_PRIVATE, stRI.MainClassID, stRI.ID, 0);
		stRI.SID = CodeID.FiniToString();
		dlgDetails.ReallySave_Attachs(ECOO_ADD);
		wyytExtendDataManagePri::Instance().InsertSysRecordInfo(stRI);
...
INT_PTR CPrivateSharedRecordsCtrl::ShowRecPPDlg(int nOpt, UINT uStartPageID)
{
