...
//加载所有支持的语言，即设置相应的bSupported是真还是假
...
{
	InitLanguages(GetMuleDirectory(WYYT_LANGDIR), GetMuleDirectory(WYYT_LANGDIR, false));

	bool bFoundLang = false;
	if (m_wLanguageID)
		bFoundLang = LoadLangLib(GetMuleDirectory(WYYT_LANGDIR), GetMuleDirectory(WYYT_LANGDIR, false), m_wLanguageID);
	...
	if (!bFoundLang)
	{
		...
		bFoundLang = LoadLangLib(GetMuleDirectory(WYYT_LANGDIR), GetMuleDirectory(WYYT_LANGDIR, false), lidLocale);
		if (!bFoundLang)
		{
			LoadLangLib(GetMuleDirectory(WYYT_LANGDIR), GetMuleDirectory(WYYT_LANGDIR, false), LANGID_ZH_CN);
	...
	// if loading a string fails, set language to Chinese
...
{
	InitLanguages(GetMuleDirectory(WYYT_LANGDIR), GetMuleDirectory(WYYT_LANGDIR, false), bUpdateBefore);
	...
	for (;pLang->lid;pLang++)
	{
		...
		{
			bool bResult = CheckLangDLLVersion(GetMuleDirectory(WYYT_LANGDIR) + CString(pLang->pszISOLocale) + _T(".dll"));
			return bResult || CheckLangDLLVersion(GetMuleDirectory(WYYT_LANGDIR, false) + CString(pLang->pszISOLocale) + _T(".dll"));
		}
	}
	return false; 
}
CString CPreferences::GetLangNameByID(LANGID lidSelected)
...
void CPreferences::InitThreadLocale()
{
	...
	if (theApp.GetProfileInt(_T("wyyt"), _T("SetLanguageACP"), 0) != 0)
	{
