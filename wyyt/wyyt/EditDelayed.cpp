...
void CEditDelayed::OnInit(EEditFilterType eFilterType)
{
...
void CEditDelayed::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	//{
		if (point.x <= ICON_LEFTSPACE)
		{
			...
			int nCount = m_mapDropdownItems.size();
			size_t ignoredColums = m_aIgnoredColums;
			for (int i = 0; i < nCount ;i++) 
			{
				
				menu.AppendMenu(MF_STRING | ((m_nCurrentColumnIdx == (size_t)i) ? MF_CHECKED : MF_UNCHECKED), MP_FILTERCOLUMNS + i, m_mapDropdownItems[i].ItemText );
			}
...
void CEditDelayed::ShowColumnText(bool bShow,bool bForce)
{
	if (bShow)
	{
		...
		if ( m_mapDropdownItems.size() > 0 )
		{
			
			SetWindowText(m_mapDropdownItems[m_nCurrentColumnIdx].ItemText);
...
void CEditDelayed::InitDropdownInfo()
{
	if ( E_EFT_RECORD  == m_eFilterType )
	{
		stDropdownItem stInfo;
		stInfo.ItemIndex = E_EFC_TITLE;
		stInfo.ItemText = L"标题";
		m_mapDropdownItems[stInfo.ItemIndex] = stInfo;
		stInfo.ItemIndex = E_EFC_CODE;
		stInfo.ItemText = L"编号";
		m_mapDropdownItems[stInfo.ItemIndex] = stInfo;
		stInfo.ItemIndex = E_EFC_SUMMARY;
		stInfo.ItemText = L"摘要";
		m_mapDropdownItems[stInfo.ItemIndex] = stInfo;
		stInfo.ItemIndex = E_EFC_QUOTE;
		stInfo.ItemText = L"引用";
		m_mapDropdownItems[stInfo.ItemIndex] = stInfo;
		stInfo.ItemIndex = E_EFC_CONTENT;
		stInfo.ItemText = L"内容";
		m_mapDropdownItems[stInfo.ItemIndex] = stInfo;
		stInfo.ItemIndex = E_EFC_ATTACH;
		stInfo.ItemText = L"附件信息";
		m_mapDropdownItems[stInfo.ItemIndex] = stInfo;
		stInfo.ItemIndex = E_EFC_ALL;
		stInfo.ItemText = L"全文";
		m_mapDropdownItems[stInfo.ItemIndex] = stInfo;
	}
