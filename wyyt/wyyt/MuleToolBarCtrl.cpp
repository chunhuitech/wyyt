...
void CMuleToolbarCtrl::Init(void)
{
	...
	lLen2 = _tcslen(GetResString(IDS_YT_SYSRES)) + 1;
	memcpy(cButtonStrings+lLen, GetResString(IDS_YT_SYSRES), lLen2*sizeof(TCHAR));
	lLen += lLen2;
	++m_buttoncount;

	lLen2 = _tcslen(GetResString(IDS_YT_GROUPRES)) + 1;
	memcpy(cButtonStrings+lLen, GetResString(IDS_YT_GROUPRES), lLen2*sizeof(TCHAR));
	lLen += lLen2;
	++m_buttoncount;

	lLen2 = _tcslen(GetResString(IDS_YT_PRIVATERES)) + 1;
	memcpy(cButtonStrings+lLen, GetResString(IDS_YT_PRIVATERES), lLen2*sizeof(TCHAR));
	lLen += lLen2;
	++m_buttoncount;
	...
	lLen2 = _tcslen(GetResString(IDS_INFOS)) + 1; //IDS_TOOLS
	memcpy(cButtonStrings+lLen, GetResString(IDS_INFOS), lLen2*sizeof(TCHAR));
	lLen += lLen2;
	++m_buttoncount;
...
void CMuleToolbarCtrl::OnNmRClick(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	...
	UINT i = 1;
	CString toolbardir = thePrefs.GetMuleDirectory(WYYT_TOOLBARDIR, false);
	...
	i = 1;
	CString skindir = thePrefs.GetMuleDirectory(WYYT_SKINDIR, false);
...
//	theApp.wyytdlg->ShowConnectionState();
...
BOOL CMuleToolbarCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	switch (wParam)
	{
		case MP_SELECTTOOLBARBITMAPDIR:{
			...
			_sntprintf(buffer, _countof(buffer) - 1, _T("%s"), thePrefs.GetMuleDirectory(WYYT_TOOLBARDIR));
			buffer[_countof(buffer) - 1] = _T('\0');
			if(SelectDir(m_hWnd, buffer, GetResString(IDS_SELECTTOOLBARBITMAPDIR)))
				thePrefs.SetMuleDirectory(WYYT_TOOLBARDIR, buffer);
			break;
		}
		...
		case MP_SELECT_SKIN_DIR:{
			TCHAR buffer[MAX_PATH];
			_sntprintf(buffer, _countof(buffer) - 1, _T("%s"), thePrefs.GetMuleDirectory(WYYT_SKINDIR));
			buffer[_countof(buffer) - 1] = _T('\0');
			if(SelectDir(m_hWnd, buffer, GetResString(IDS_SELSKINPROFILEDIR)))
				thePrefs.SetMuleDirectory(WYYT_SKINDIR, buffer);
			break;
		}
...
void CMuleToolbarCtrl::Refresh()
{
	...
	if (theApp.wyytdlg->m_ctlMainTopReBar.m_hWnd)
	{
	    theApp.wyytdlg->RemoveAnchor(theApp.wyytdlg->m_ctlMainTopReBar.m_hWnd);
	    ...
	    VERIFY( theApp.wyytdlg->m_ctlMainTopReBar.SetBandInfo(MULE_TOOLBAR_BAND_NR, &rbbi) );

	    theApp.wyytdlg->AddAnchor(theApp.wyytdlg->m_ctlMainTopReBar.m_hWnd, TOP_LEFT, TOP_RIGHT);
	}
	...
	else
	{
		...
		CRect rClientRect;
		theApp.wyytdlg->GetClientRect(&rClientRect);

		CRect rStatusbarRect;
		theApp.wyytdlg->statusbar->GetWindowRect(&rStatusbarRect);
		...
		CWnd* wnds[] =
		{
			theApp.wyytdlg->priresmanagewnd,
			theApp.wyytdlg->sysresmanagewnd,
			theApp.wyytdlg->groupresmanagewnd,
			theApp.wyytdlg->sharedfileswnd,
			theApp.wyytdlg->searchwnd,
			theApp.wyytdlg->chatwnd,
			theApp.wyytdlg->ircwnd,
			theApp.wyytdlg->statisticswnd
		};
		for (int i = 0; i < _countof(wnds); i++)
		{
			...
			theApp.wyytdlg->RemoveAnchor(wnds[i]->m_hWnd);
			theApp.wyytdlg->AddAnchor(wnds[i]->m_hWnd, TOP_LEFT, BOTTOM_RIGHT);
		}
		theApp.wyytdlg->Invalidate();
		theApp.wyytdlg->RedrawWindow();
	}
}
...
{
	if (theApp.wyytdlg->m_ctlMainTopReBar)
	{
		...
		if (hbmp)
		{
			...
			if (theApp.wyytdlg->m_ctlMainTopReBar.GetBandInfo(MULE_TOOLBAR_BAND_NR, &rbbi))
			{
				rbbi.fMask = RBBIM_STYLE | RBBIM_BACKGROUND;
				rbbi.fStyle |= RBBS_FIXEDBMP;
				rbbi.hbmBack = hbmp;
				if (theApp.wyytdlg->m_ctlMainTopReBar.SetBandInfo(MULE_TOOLBAR_BAND_NR, &rbbi))
				{
					if (m_bmpBack.m_hObject)
						VERIFY( m_bmpBack.DeleteObject() );
...
BOOL CMuleToolbarCtrl::PreTranslateMessage(MSG* pMsg)
{
	// TODO: 在此添加专用代码和/或调用基类
#ifdef _PUBLISH_
	if(pMsg->message == WM_LBUTTONDBLCLK /*WM_NCLBUTTONDBLCLK*/)   //去除双击功能
	{
		return TRUE;
	}
#endif // _PUBLISH_
